...
7. **建立 view／mview 定義。**
   ...
   view 與 mview 的名稱上限都是 **30 個字元**，以字元數而非位元組數計算。
   建立前先檢查最終名稱；`list_views`、`get_view` 與 `create_view` 必須使用同一個
   已通過長度檢查的名稱。使用者指定的名稱超過 30 個字元時，不得直接建立或靜默
   截斷；提出保留原意且不超過 30 個字元的替代名稱，取得使用者確認後才繼續。
   使用者未指定名稱時，直接產生語意清楚且不超過 30 個字元的名稱。
