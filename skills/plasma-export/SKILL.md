...
name: plasma-export
description: >-
  當使用者希望將表單、報表、BI 指標或查詢結果直接寫入外部資料庫（PostgreSQL 等）指定資料表時使用。查核知識與 Trino SQL，建立 view，在 blueprint 選取此 view 並設定目的地，再執行及驗證同步。需要資料 API 時改用 plasma-data-api。
---

# 將 BI 資料透過 view → blueprint → 外部資料表匯出

## 目標與互動原則

固定流程為：**建立 view → 設定 blueprint 並選取此 view → 拉到目標資料表**。
使用一般 `view`，不先建立 `mview`，也不開啟資料 API。

全程使用台灣繁體中文，保留 view、mview、blueprint、workspace 等系統功能名稱，
...
整理所有欄位、指標、粒度、期間與篩選。**目的地連線必須反問使用者選擇，不自行代選。**

先呼叫 `overview`，從目前 workspace 的 DB 清單整理已與 Plasma 串接的候選 DB。
**`dbc_id` 來自知識工具** —— 每個 relation 都帶著它，`get_table_card` 會給。
沒有列出資料庫連線的工具，也不需要；不要杜撰 `list_pg_connections`、
`list_databases` 這類不存在的工具。

以表格列出可辨識的選項，反問：「這次要把資料寫到哪個連線？」即使只有一個
候選，也請使用者確認選擇，不預設使用來源 DB、第一筆或名稱最相近的連線。

沿用使用者在本次任務已明確選定的連線，不重複問。尚未選定時可繼續來源查核、
SQL 驗證與 view 建立，但不可建立 blueprint 或寫入目的地。選項重名或對應不明時
集中釐清，不能把名稱相似當成同一個 DB。取不到清單時說明原因，仍由使用者選擇。

目前使用既有 DBC，不收集或回傳連線密碼；尚未串接的資料庫需先在 Plasma 建立
連線，不能自行改用臨時連線。
...
依 [知識查找技能](../ophion-knowledge-lookup/SKILL.md) 查明來源與證據，
並共用 [資料 API 技能的步驟 1–6](../plasma-data-api/SKILL.md) 完成逐欄、概念、
代碼、規則、存取模式與 SQL 查核。只共用查核步驟，不接續建立 mview 或發布 API。

來源查詢使用 Trino 方言，即使目的地是 PostgreSQL 也不改寫為該資料庫的語法。以 `run_query`
...
已存在，不代表資料已寫入目的地。不要呼叫 `sync_view` 或等待 mview 的
`last_sync_status=synced`；資料搬移由 blueprint 執行。

## 4. 設定 blueprint 與目的地

先用 `list_blueprints` 分頁搜尋既有 user query blueprint，再以 `get_blueprint`
核對來源 `source_view_id`、目的地 DBC、目標表、寫入模式與是否允許建表。只有完整
符合需求才重用，不能因名稱相同就執行既有 blueprint。

新建使用 `create_export_blueprint`，提供 `name`、`view_id`、`dbc_id`、`table`、
`write_mode`、`force_create_table`，可加 `description`。
`table` 是單一資料表名稱，不加 schema 前綴或 SQL 引號。

**沒有 `database` 與 `schema` 參數，這是刻意的。** 工具以 `source_view_id`
綁定 view，後端執行時從 view 的 path 組成來源查詢，不複製來源 SQL；目的地則由
後端從 `dbc_id` 解析 catalog、schema、主機與憑證。呼叫端給不了、也不需要給這些
座標。若使用者要求的 database／schema 與該 DBC 不符，請他選另一個 DBC，
不能假裝已切換。
...
啟動前一次呈現 workspace、view、blueprint、完整目的地、欄位範圍、日期
與篩選、SQL 驗證結果、寫入方式、是否允許建表及候選假設。明確說明會從來源拉取資料、
使用資源並寫入指定的目的地資料表；涉及覆寫或刪除時指出對既有資料的影響。
...
向目的地寫入代替 blueprint。若工具呼叫逾時，先查 jobs 判斷是否已受理，不直接重送。
...
透過已授權且可用的目的地查詢方式，或使用者提供的驗證結果，核對實際目標表、
欄位、筆數與代表性資料。工具沒有直接查詢目的地的功能；未查核時，分別說明
「後端 job 已完成」與「目的地資料內容尚未另行驗證」。來源預覽不能證明目的地內容。

交付 view／blueprint ID、目的地資料庫／綱要／資料表、寫入方式、更新頻率、執行
狀態及驗證結果。無法查核目的地時明列待驗證項目，不宣稱全部完成。提供 BI
工具連至該目的地資料表的資訊，不交付資料 API URL。
